...
import org.setup.Listify.Assembler.ProjectModelAssembler;
import org.setup.Listify.Model.Projects;
import org.setup.Listify.Service.ProjectService;
import org.springframework.http.HttpStatus;
import org.springframework.http.ResponseEntity;
...
public class ProjectController {
    ...
    @PostMapping("/create")
    public ResponseEntity<Void> createProject(@RequestParam Long teamID,
    ...
    // Added global exception handler for RuntimeException
    @ExceptionHandler(RuntimeException.class)
    public ResponseEntity<String> handleRuntimeException(RuntimeException ex) {
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(ex.getMessage());
    }

    // Added exception handler for method argument validation errors
    @ExceptionHandler(MethodArgumentNotValidException.class)
    public ResponseEntity<String> handleMethodArgumentNotValid(MethodArgumentNotValidException ex) {
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Invalid input data");
    }
