...
export async function GetDueTasksForProject<dueTasks>(jwtToken: string|null,projectID:string): Promise<dueTasks | null> {
  try {
    const response = await fetch("http://localhost:8080/projects/"+projectID+"/dueTasks", {
      headers: {
        "Authorization": `Bearer ${jwtToken}`,
      },
    });
